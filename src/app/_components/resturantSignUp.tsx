...
const ResturantSignUp = () => {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    // console.log(data);
    const { email, password, cpassword, rname, city, address, phone } = data;
    if (password === cpassword) {
      setError(false);
      let response = await fetch("http://localhost:3000/api/resturant", {
        method: "POST",
        body: JSON.stringify({ email, password, rname, city, address, phone }),
      });
  ...
  return (
    <div className="rest-log">
      <div className="login">
        <h3 className="login-header">SignUp Page</h3>
